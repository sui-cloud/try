...
def penguin():
    ...
    X = dataset[:,:-1]
    y = dataset[:,-1]
    st.write('특징모양:', X.shape)
    st.write('타깃모양:', y.shape)
    #데이터 정규화하기
    scaler = MinMaxScaler()
    ...
    st.write("훈련 데이터:", X_train.shape, y_train.shape)
    st.write("테스트 데이터:", X_test.shape, y_test.shape)
    #모델 생성하기-최근접 이웃 알고리즘으로 학습하기
    knn = KNeighborsClassifier(n_neighbors=7)
    knn.fit(X_train, y_train)
    st.write("훈련 데이터를 이용한 모델 분류 정확도:", knn.score(X_train, y_train))
    #모델 평가 및 예측하기
    #모델 평가하기
    st.write("테스트 데이터를 이용한 모델 성능 평가:", knn.score(X_test, y_test))
    #최적의 K구하기
    for k in range(2,11):
        knn = KNeighborsClassifier(n_neighbors=k)
        knn.fit(X_train, y_train)
        score = knn.score(X_test, y_test)
        st.write('k: %d, accuracy: %2f' %(k, score*100))
        #테스트 데이터 예측하기
